...
**Resuming `in-progress`/`ready-to-deploy` (the out-of-band-deploy trap).** The deploy that completes a level happens in the terminal and you can't see it, so a marker mid-Level-N may mean they've since deployed. **Don't pick up as if still modding — confirm:** *"Welcome back — last time you'd modded your Level N version and were about to deploy. Did you deploy it? (I can't see deploys — they're in your terminal.)"* Deployed → mark `deployed`, move to **Level N+1**. Not yet → stay on N, offer to deploy. If their message already answers it ("deployed" / "let's keep tweaking"), act on it without asking.
