...
const domain = process.env?.APP_DOTNS_DOMAIN ?? "playground-tutorial";
const label = domain.toLowerCase().replace(/\.dot$/, "");

export default defineConfig({
  domain: `${label}.dot`,
  displayName: "Playground Tutorial",
  description:
    "A guided, hands-on tutorial that turns a plain Rock Paper Scissors web app into a full on-chain Polkadot product, one layer of the stack at a time.",
